# -*- coding: utf-8 -*-
"""
非洲域名测试列表 —— 批量 DNS 解析 + 运营商简化 + ping 时延/丢包测试（Linux）
...
与 Windows 版的差异（其余逻辑完全一致，便于结果对拍）：
  * ping 用 Linux/iputils 语法：ping -c <报文数> -W <单包超时秒> -i <间隔秒> <IP>；
  * 解析 Linux ping 摘要（rtt min/avg/max/mdev = ...、N% packet loss），
    同时保留对 Windows 中英文输出的解析能力；
  * dig / ping 的输出按系统区域编码解码（Linux 通常为 UTF-8）；
  * 子进程不再传 creationflags（Windows 专用参数）。

用法：
    ./run_dns_test.sh                        # 一键运行（自动准备 Python/依赖/dig）
    python3 domain_dns_test.py                # 处理全部域名，结果写入 *_result.xlsx
    python3 domain_dns_test.py --limit 5      # 只处理前 5 个域名（试跑，默认不写表尾统计）
    python3 domain_dns_test.py --dry-run      # 只打印，不写 Excel
    python3 domain_dns_test.py --inplace      # 直接回写原文件（默认另存为新文件）
    python3 domain_dns_test.py --no-ping      # 跳过 ping（不写时延/丢包率）
"""
...
PING_TIMEOUT_MS = 4000              # 单个报文的等待超时（毫秒；Linux 下换算成 ping -W 的秒）
PING_WORKERS = 16                   # ping 并发数
PING_INTERVAL_SEC = 1.0             # 报文间隔秒数（ping -i；Windows 版固定为 1 秒，保持一致）
...
def subprocess_kwargs() -> Dict[str, Any]:
    """
    平台相关的 subprocess.run 参数。

    creationflags 只存在于 Windows（用于隐藏弹出的控制台窗口）；
    在 Linux 上传该参数会直接抛 TypeError，所以这里按平台返回。
    """
...
def decode_console(data: bytes) -> str:
    ...
    Linux 下命令输出跟随系统区域设置（通常是 UTF-8，某些容器/精简系统是 POSIX/ASCII），
    中文 Windows 下则是控制台 OEM 代码页（cp936）。按「系统首选编码 → UTF-8 → latin-1」
    依次尝试，避免按错误编码硬解出现乱码。
    """
    if not data:
        return ""
    ...
    if IS_WINDOWS:
        ...
        try:
            encodings.append(locale.getpreferredencoding(False) or "")
        except Exception:
            pass
...
def find_dig_executable() -> str:
    ...
    查找顺序：PATH → 程序目录下的便携版（tools/bind/dig 或 ./dig）
    → 常见安装位置（/usr/bin、/usr/local/bin、/snap/bin 等）。
    便携版目录里通常还有 dig 依赖的动态库，整个目录拷贝过来即可免安装使用。
    """
    exe = shutil.which("dig")
    if exe:
        return exe

    for portable in (BASE_DIR / "tools" / "bind" / "dig",
                     BASE_DIR / "tools" / "bind" / "dig.exe",
                     BASE_DIR / "dig"):
        if portable.exists():
            return str(portable)

    for candidate in ("/usr/bin/dig", "/bin/dig", "/usr/local/bin/dig",
                      "/usr/sbin/dig", "/sbin/dig", "/snap/bin/dig",
                      "/opt/homebrew/bin/dig"):
        if os.path.exists(candidate):
            return candidate

    for pattern in ("/usr/local/bind*/bin/dig", "/opt/bind*/bin/dig"):
    ...
    raise FileNotFoundError(
        ...
        "    1) 自动安装：运行 ./run_dns_test.sh（脚本会用 apt/dnf/yum/apk 安装 dnsutils）；\n"
        "    2) 手动安装：Debian/Ubuntu  sudo apt install -y dnsutils\n"
        "                 RHEL/CentOS   sudo dnf install -y bind-utils\n"
        "                 Alpine        sudo apk add bind-tools\n"
        "    3) 免安装：把另一台机器上可用的 dig 及其依赖放到\n"
        "       <本程序目录>/tools/bind/ 下（确保有可执行权限）"
    )
...
PING_LOSS_RE = re.compile(r"\((\d+)\s*%")                      # Windows：(0% 丢失) / (20% loss)
PING_LOSS_LINUX_RE = re.compile(r"(\d+(?:\.\d+)?)\s*%\s*packet\s+loss",
                                re.IGNORECASE)                      # Linux：0% packet loss
PING_AVG_RE = re.compile(r"(?:平均|Average|average)\s*=\s*<?\s*(\d+(?:[.,]\d+)?)\s*ms")
PING_RTT_RE = re.compile(r"(?:rtt|round-trip|round trip)\s+min/avg/max(?:/mdev|/stddev)?\s*="
...
def parse_ping_output(text: str) -> Tuple[Optional[float], Optional[int]]:
    """
    从 ping 输出中解析出 (平均时延 ms, 丢包率 %)。

    Linux（iputils / busybox）：
        "rtt min/avg/max/mdev = 22.100/23.456/25.000/1.234 ms"
        "10 packets transmitted, 10 received, 0% packet loss, time 9000ms"
    Windows 的中英文输出也一并支持，方便两个版本的代码/结果对拍：
    ...
    for line in (text or "").splitlines():
        if loss_percent is None:
            match = PING_LOSS_LINUX_RE.search(line) or PING_LOSS_RE.search(line)
            if match:
                loss_percent = int(round(float(match.group(1).replace(",", "."))))
        if avg_ms is None:
            match = PING_RTT_RE.search(line)
            if match:
                avg_ms = float(match.group(2).replace(",", "."))
            else:
...
def find_ping_executable() -> str:
    """定位 ping（不依赖 PATH，PATH 被改坏的机器也能用）。"""
    exe = shutil.which("ping")
    if exe:
        return exe
    for candidate in ("/bin/ping", "/usr/bin/ping", "/sbin/ping",
                      "/usr/sbin/ping", "/usr/local/bin/ping"):
        if os.path.exists(candidate):
            return candidate
    return "ping"


def build_ping_command(ping_exe: str, ip: str, count: int, timeout_ms: int,
                       interval_sec: float = PING_INTERVAL_SEC) -> List[str]:
    """
    拼出 Linux ping 命令：ping -c <报文数> -W <单包超时秒> -i <间隔秒> <IP>。

    Linux 的 -W 单位是秒（Windows 的 -w 是毫秒），而参数仍然用毫秒表达，
    所以向上取整到秒，最少 1 秒。-w/--deadline 不用是因为整体上限在 Python 侧控制。
    """
    per_reply_seconds = max(1, int(math.ceil(timeout_ms / 1000.0)))
    return [ping_exe, "-c", str(count), "-W", str(per_reply_seconds),
            "-i", f"{interval_sec:g}", ip]


def describe_ping_failure(text: str, returncode: int) -> str:
    """把 ping 的失败输出翻译成可读原因（Linux 上最常见的两类：无权限 / 不可达）。"""
    lowered = (text or "").lower()
    if "not permitted" in lowered or "permission denied" in lowered:
        return ("无权限发送 ICMP（需要 root 或 CAP_NET_RAW；可执行 "
                "sudo setcap cap_net_raw+ep $(which ping)，"
                "或 sysctl -w net.ipv4.ping_group_range='0 2147483647'）")
    if "network is unreachable" in lowered:
        return "网络不可达"
    if "name or service not known" in lowered or "bad address" in lowered:
        return "无法解析目标地址"
    first_line = next((line.strip() for line in (text or "").splitlines() if line.strip()), "")
    if first_line:
        return f"无法解析 ping 输出：{first_line[:80]}"
    return f"ping 无输出（退出码 {returncode}）"


def ping_ip(ip: str, ping_exe: str, count: int = PING_COUNT,
            timeout_ms: int = PING_TIMEOUT_MS,
            interval_sec: float = PING_INTERVAL_SEC) -> PingResult:
    """对单个 IP 执行一次 ping（count 个报文），返回平均时延与丢包率。"""
    command = build_ping_command(ping_exe, ip, count, timeout_ms, interval_sec)
    process_timeout = count * (max(interval_sec, 0.2) + max(1, timeout_ms / 1000.0)) + 15
    try:
    ...
    if avg_ms is None and loss_percent is None:
        return PingResult(ip=ip, error=describe_ping_failure(text, proc.returncode), count=count)
...
def ping_ip_with_retry(ip: str, ping_exe: str, count: int = PING_COUNT,
                       ...
                       interval_sec: float = PING_INTERVAL_SEC
...
def run_pings(ips: Sequence[str], ping_exe: str, cache: PingCache, workers: int,
              ...
              retry_on_loss: bool = PING_RETRY_ON_LOSS,
              interval_sec: float = PING_INTERVAL_SEC) -> Dict[str, PingResult]:
    """
    ...
    if total:
        ...
        print(f"[信息] 需要 ping 的 IP：{total} 个（每个 {count} 个报文、单包超时 {timeout_ms}ms、"
              f"间隔 {interval_sec:g}s、并发 {workers}{retry_note}；"
              f"缓存命中 {len(results)} 个）")
    done = 0
    ...
    def work(ip: str) -> Tuple[PingResult, Optional[str]]:
        return ping_ip_with_retry(ip, ping_exe, count=count, timeout_ms=timeout_ms,
                                  retry_on_loss=retry_on_loss, interval_sec=interval_sec)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--dig", default=None, help="dig 可执行文件路径（默认自动查找）")
    ...
    parser.add_argument("--ping-timeout", type=int, default=PING_TIMEOUT_MS,
                        help=f"单个 ping 报文的等待超时（毫秒，默认 {PING_TIMEOUT_MS}；"
                             f"Linux 下会换算成整数秒传给 ping -W）")
    parser.add_argument("--ping-interval", type=float, default=PING_INTERVAL_SEC,
                        help=f"报文间隔秒数（默认 {PING_INTERVAL_SEC:g}，与 Windows 版一致；"
                             f"调小可加快测试，非 root 下最小 0.2）")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if ping_exe:
        ...
        print(f"[信息] ping 路径：{ping_exe}（每个 IP {args.ping_count} 个报文，"
              f"单包超时 {args.ping_timeout}ms，间隔 {args.ping_interval:g}s，"
              f"并发 {args.ping_workers}；{retry_text}）")
    else:
    ...
    if ping_exe and all_ips:
        ...
        pings = run_pings(all_ips, ping_exe, ping_cache, args.ping_workers,
                          ...
                          interval_sec=args.ping_interval)
...
if __name__ == "__main__":
    # Linux 终端一般为 UTF-8；显式固定编码，避免被 LANG=C 之类环境影响，
    # errors="replace" 保证存在无法编码的字符时也不会崩溃
    try:
