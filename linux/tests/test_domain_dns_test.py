...
class TestHelpers(unittest.TestCase):
    ...
    def test_decode_console_handles_console_encoding(self):
        """dig / ping 的输出按系统区域编码解码，UTF-8 与纯 ASCII 都要能还原。"""
        text = "平均 = 23ms"
        if not app.IS_WINDOWS:
            # Linux 下首选编码就是 UTF-8，中文必须原样还原
            self.assertEqual(app.decode_console(text.encode("utf-8")), text)
        ascii_text = "rtt min/avg/max = 1/2/3 ms"
        self.assertEqual(app.decode_console(ascii_text.encode("ascii")), ascii_text)
        self.assertEqual(app.decode_console(b""), "")
        # 无法解码的字节序列不能抛异常
        self.assertIsInstance(app.decode_console(b"\xff\xfe\x00abc"), str)

    def test_subprocess_kwargs_has_no_windows_only_flags_on_posix(self):
        """Linux 上传 creationflags 会直接报 TypeError，必须只在 Windows 上传。"""
        kwargs = app.subprocess_kwargs()
        if app.IS_WINDOWS:
            self.assertEqual(kwargs["creationflags"], app.CREATE_NO_WINDOW)
        else:
...
# ---------- Linux（iputils / busybox）的真实输出样例 ----------

LINUX_PING_OK = """PING 8.8.8.8 (8.8.8.8) 56(84) bytes of data.
64 bytes from 8.8.8.8: icmp_seq=1 ttl=117 time=22.1 ms
64 bytes from 8.8.8.8: icmp_seq=2 ttl=117 time=25.0 ms

--- 8.8.8.8 ping statistics ---
2 packets transmitted, 2 received, 0% packet loss, time 1001ms
rtt min/avg/max/mdev = 22.100/23.550/25.000/1.450 ms
"""

LINUX_PING_PARTIAL_LOSS = """PING 1.2.3.4 (1.2.3.4) 56(84) bytes of data.
64 bytes from 1.2.3.4: icmp_seq=1 ttl=52 time=40.2 ms
64 bytes from 1.2.3.4: icmp_seq=2 ttl=52 time=50.8 ms

--- 1.2.3.4 ping statistics ---
10 packets transmitted, 8 received, 20% packet loss, time 9020ms
rtt min/avg/max/mdev = 40.200/45.500/60.000/5.100 ms
"""

LINUX_PING_ALL_LOST = """PING 10.255.255.1 (10.255.255.1) 56(84) bytes of data.

--- 10.255.255.1 ping statistics ---
10 packets transmitted, 0 received, 100% packet loss, time 9226ms
"""

# BusyBox ping（Alpine 等精简发行版）的摘要行写法不同：round-trip 而不是 rtt
BUSYBOX_PING_OK = """PING 1.1.1.1 (1.1.1.1): 56 data bytes
...
# 非 root 且未放开 ping_group_range 时最常见的失败
PING_PERMISSION_DENIED = "ping: socket: Operation not permitted\n"
...
class TestPingParsing(unittest.TestCase):
    ...
    def test_linux_summary(self):
        """rtt min/avg/max/mdev 行取 avg（第二列），丢包率取 "N% packet loss"。"""
        self.assertEqual(app.parse_ping_output(LINUX_PING_OK), (23.55, 0))

    def test_linux_partial_loss(self):
        self.assertEqual(app.parse_ping_output(LINUX_PING_PARTIAL_LOSS), (45.5, 20))

    def test_linux_all_lost(self):
        self.assertEqual(app.parse_ping_output(LINUX_PING_ALL_LOST), (None, 100))

    def test_busybox_summary(self):
        """BusyBox 写的是 round-trip min/avg/max（没有 mdev）。"""
        self.assertEqual(app.parse_ping_output(BUSYBOX_PING_OK), (12.345, 0))

    def test_linux_fallback_to_reply_lines(self):
        """摘要行缺失（例如被中断）时用每个回包的 time= 求平均。"""
        text = ("64 bytes from 1.2.3.4: icmp_seq=1 ttl=52 time=20.0 ms\n"
                "64 bytes from 1.2.3.4: icmp_seq=2 ttl=52 time=30.0 ms\n")
    ...
    def test_describe_ping_failure_permission(self):
        """没有 CAP_NET_RAW 时给出可操作的提示，而不是丢一句原始报错。"""
        reason = app.describe_ping_failure(PING_PERMISSION_DENIED, 2)
        self.assertIn("CAP_NET_RAW", reason)
        self.assertIn("setcap", reason)

    def test_describe_ping_failure_other(self):
        self.assertEqual(app.describe_ping_failure("ping: connect: Network is unreachable", 2),
                         "网络不可达")
        self.assertIn("无法解析目标地址",
                      app.describe_ping_failure("ping: nope: Name or service not known", 2))
        self.assertIn("退出码 3", app.describe_ping_failure("", 3))
    ...
    def test_ping_ip_parses_command_output(self):
        proc = SimpleNamespace(stdout=LINUX_PING_PARTIAL_LOSS.encode("utf-8"),
                               stderr=b"", returncode=0)
        with mock.patch.object(app.subprocess, "run", return_value=proc) as run:
            result = app.ping_ip("1.2.3.4", "ping", count=10, timeout_ms=1000)
        self.assertEqual((result.avg_ms, result.loss_percent), (45.5, 20))
        command = run.call_args[0][0]
        # Linux 语法：-c 报文数、-W 单包超时（秒，由毫秒向上取整）、-i 报文间隔
        self.assertEqual(command, ["ping", "-c", "10", "-W", "1", "-i", "1", "1.2.3.4"])

    def test_build_ping_command_rounds_timeout_up_to_seconds(self):
        self.assertEqual(app.build_ping_command("ping", "1.2.3.4", 5, 4000),
                         ["ping", "-c", "5", "-W", "4", "-i", "1", "1.2.3.4"])
        self.assertEqual(app.build_ping_command("ping", "1.2.3.4", 5, 4001),
                         ["ping", "-c", "5", "-W", "5", "-i", "1", "1.2.3.4"])
        self.assertEqual(app.build_ping_command("ping", "1.2.3.4", 3, 200, 0.2),
                         ["ping", "-c", "3", "-W", "1", "-i", "0.2", "1.2.3.4"])

    def test_ping_ip_reports_permission_error(self):
        proc = SimpleNamespace(stdout=b"", stderr=PING_PERMISSION_DENIED.encode(),
                               returncode=2)
        with mock.patch.object(app.subprocess, "run", return_value=proc):
            result = app.ping_ip("1.2.3.4", "ping", count=10)
        self.assertIsNotNone(result.error)
        self.assertIn("CAP_NET_RAW", result.error)
