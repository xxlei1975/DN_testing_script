...
def main(path_str: str) -> int:
    ...
    print(f"数据区：{layout.data_start_row}~{layout.data_end_row}，共 {len(domains)} 行")
    ...
    row_latency: Dict[int, Dict[str, List[str]]] = {}

    for row, domain in domains:
        seen_domains.setdefault(domain.lower(), []).append(row)
        row_latency[row] = {}
        for target in layout.targets:
            cname_cell = ws.cell(row=row, column=target.cname_col)
            a_cell = ws.cell(row=row, column=target.a_col)
            ips = lines_of(a_cell.value)
            attrs = lines_of(ws.cell(row=row, column=target.ip_attr_col).value) \
                if target.ip_attr_col else []
            latencies = lines_of(ws.cell(row=row, column=target.latency_col).value) \
                if target.latency_col else []
            losses = lines_of(ws.cell(row=row, column=target.loss_col).value) \
                if target.loss_col else []
            first_attr = ws.cell(row=row, column=target.first_ip_attr_col).value \
                if target.first_ip_attr_col else None
            first_text = str(first_attr).strip() if first_attr is not None else ""
            row_latency[row][target.ip] = latencies

            if not ips:
                filled["invalid"] += 1
                per_server[id(target)]["empty"] += 1
                for cell in (cname_cell, a_cell):
                    if cell.value not in (None, ""):
                        problems.append(f"{cell.coordinate} 无解析结果的行不应有内容：{cell.value!r}")
                if any(latencies) or any(losses):
                    problems.append(f"第{row}行 @{target.ip}：无解析结果却有 ping 数据")
                continue
            ...
            for ip in ips:
                if not app.is_ip_address(ip):
                    problems.append(f"{a_cell.coordinate} A 列应只写纯 IP：{ip!r}")
            if not a_cell.alignment.wrap_text:
                problems.append(f"{a_cell.coordinate} A 单元格未设置自动换行")
            ...
            if attrs:
                ...
                if len(attrs) != len(ips):
                    problems.append(f"第{row}行 @{target.ip}：IP归属 {len(attrs)} 行与 "
                                    f"A 的 {len(ips)} 个 IP 不对应")
                if first_text != attrs[0]:
                    problems.append(f"第{row}行 @{target.ip}：首IP归属 {first_text!r} 与 "
                                    f"IP归属第一行 {attrs[0]!r} 不一致")
            elif first_text:
                problems.append(f"第{row}行 @{target.ip}：没有 IP归属 却有首IP归属 {first_text!r}")

            if latencies:
                filled["latency"] += 1
                if len(latencies) != len(ips):
                    problems.append(f"第{row}行 @{target.ip}：时延 {len(latencies)} 行与 IP 数不对应")
                for line in latencies:
                    if line != app.PING_PLACEHOLDER and not LATENCY_RE.match(line):
                        problems.append(f"第{row}行 @{target.ip}：时延格式异常 {line!r}")
            if losses:
                filled["loss"] += 1
                if len(losses) != len(ips):
                    problems.append(f"第{row}行 @{target.ip}：丢包率 {len(losses)} 行与 IP 数不对应")
                for line in losses:
                    if line != app.PING_PLACEHOLDER and not LOSS_RE.match(line):
                        problems.append(f"第{row}行 @{target.ip}：丢包率格式异常 {line!r}")

    # ---- 时延改善 / 丢包改善：只有「首IP归属 = CM」的行才有值 ----
    ...
    if improve_target is not None and improve_target.latency_improve_col:
        for row, _domain in domains:
            first_attr = ws.cell(row=row, column=improve_target.first_ip_attr_col).value
            latency_cell = ws.cell(row=row, column=improve_target.latency_improve_col)
            loss_cell = ws.cell(row=row, column=improve_target.loss_improve_col) \
            ...
            if str(first_attr).strip() == app.CM_CODE:
                if latency_value is None and loss_value is None:
                    has_both = bool(row_latency[row].get(improve_target.ip)) and \
                        bool(row_latency[row].get(layout.public_target.ip)
                             if layout.public_target else False)
                    if has_both:
                        problems.append(f"第{row}行：首IP归属为 CM 且两侧都有 ping 数据，"
                                        f"但改善列为空")
                else:
                    filled["improve"] += 1
                ...
                problems.append(f"第{row}行：首IP归属不是 CM（{first_attr!r}），"
    ...
    # ---- 表结构：重复域名、表头、DNS 行 ----
    print("\n逐行写入情况：")
    print(f"  有 A 记录的解析 {filled['a']} 个，无解析结果 {filled['invalid']} 个；"
          f"IP归属 {filled['attr']} 个、时延 {filled['latency']} 个、丢包率 {filled['loss']} 个、"
          f"改善值 {filled['improve']} 个")
    for target in layout.targets:
        item = per_server[id(target)]
        print(f"  {target.label:<24} 有结果 {item['filled']:>3} 行，无解析结果 {item['empty']:>3} 行")
    ...
    print("\n抽查：")
    for row in [layout.data_start_row, layout.data_start_row + 1, layout.data_end_row]:
        if row > ws.max_row:
            continue
        print(f"  第{row}行  {ws.cell(row=row, column=domain_col).value}")
        for target in layout.targets:
            print(f"    @{target.ip}"
                  f"  A={lines_of(ws.cell(row=row, column=target.a_col).value)}"
                  f"  归属={lines_of(ws.cell(row=row, column=target.ip_attr_col).value)}"
                  f"  时延={lines_of(ws.cell(row=row, column=target.latency_col).value)}"
                  f"  丢包={lines_of(ws.cell(row=row, column=target.loss_col).value)}")
